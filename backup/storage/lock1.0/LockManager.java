...
//===========================lock=======begin=====================================
     ...
     * @param tid ： 當前事務
     * @param pid： 需要鎖的page
     * @return true：tid的pid已經有讀鎖 / tid上的pid已經有寫鎖或者沒有鎖
     *         false：tid給pid加不上讀鎖，wait住了
     */
    public synchronized boolean grantSLock(TransactionId tid,PageId pid){
        List<LockState> lockStates = (ArrayList<LockState>)lockStateMap.get(pid);
        if (lockStates != null && lockStates.size()!= 0 ) {
            //如果pid上只有個一把鎖
            if (lockStates.size() == 1) {
                //拿到這把鎖
                LockState next = lockStates.iterator().next();
                //如果是當前事務所持有
                if (next.getTid().equals(tid)) {
                    //如果是當前事務在pid上持有讀鎖，那直接返回，否則加鎖
                    return next.getPerm() == Permissions.READ_ONLY ||
                            lock(pid, tid, Permissions.READ_ONLY);
                } else {
                    //如果當前事務的鎖，那麼需要加鎖，如果是寫鎖，需要等待資源釋放
                    return next.getPerm() == Permissions.READ_ONLY ?
                            lock(pid, tid, Permissions.READ_ONLY) :
                            wait(tid, pid);
                }
                ...
                //當多lock 情況出現
                /**
                 *  1, two locks ,both are belong to this tid , one slock and one xlock
                 *  2, two locks , neither are belong to this tid,one s one x
                 *  3, many slocks , one of them is tid
                 *  4, many slocks , none is this tid
                 */

                for (LockState lock : lockStates) {
                    if (lock.getPerm() == Permissions.READ_WRITE) {
                        //如果其中有一个写锁，那么根据是否为自己的来判断属于情况1还是2
                        return lock.getTid().equals(tid) || wait(tid, pid);
                    } else if (lock.getTid().equals(tid)) {//如果是读锁且是tid的
    ...
    public synchronized boolean grantXLock(TransactionId tid,PageId pid){
        List<LockState> lockStates = (ArrayList<LockState>)lockStateMap.get(pid);
        if (lockStates!=null && lockStates.size() == 0 ){
            //when there is only one lock
            if (lockStates.size() == 1){
                LockState next = lockStates.iterator().next();
                return next.getTid().equals(tid) ? next.getPerm() == Permissions.READ_WRITE ||
     ...
     * @return
     */
    private synchronized boolean lock(PageId pid, TransactionId tid, Permissions lockType) {
        LockState lockState = new LockState(tid, lockType);
